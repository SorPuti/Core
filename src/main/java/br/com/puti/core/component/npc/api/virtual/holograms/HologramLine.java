  ...
  private ArmorHologram createEntity(Location location, String text) {
    ...
    try {
      ...
      if (!location.getWorld().isChunkLoaded(chunkX, chunkZ)) {
        Core.getPlugin(Core.class).getLogger().info("Falha ao spawnar holograma, chunk nao carregada!");
        return null;
      }
      ...
      if (entity.world.addEntity(entity, SpawnReason.CUSTOM)) {
    	...
    	Core.getPlugin(Core.class).getLogger().info("Falha ao spawnar holograma: " + e.getMessage());
      return null;
    }
  }
